...
Sprite* Sprite::Create(UINT texNumber, DirectX::XMFLOAT2 anchor, bool flipX, bool flipY)
{
    Sprite* instance = new Sprite();

    instance->Initialize(texNumber, anchor, flipX, flipY);

    return instance;
}

void Sprite::Initialize(UINT texNumber, XMFLOAT2 anchor, bool flipX, bool flipY)
{
    HRESULT result = S_FALSE;

    // メンバ変数書き込み
    textureNumber = texNumber;
    anchorpoint = anchor;
    isFlipX = flipX;
    isFlipY = flipY;
...
void Sprite::Draw()
{
    ...
    // ルートパラメータ0に定数バッファをセット
    cmdList->SetGraphicsRootConstantBufferView(0, constBuff->GetGPUVirtualAddress());

    // ルートパラメータ1にシェーダリソースビューをセット
    spriteCommon->SetGraphicsRootDescriptorTable(1, textureNumber);
...
void Sprite::TransferVertexBuffer()
{
    ...
    // 指定番号の画像が読み込み済みなら
    if (spriteCommon->GetTexBuff(textureNumber)) {
        // テクスチャ情報取得
        D3D12_RESOURCE_DESC resDesc = spriteCommon->GetTexBuff(textureNumber)->GetDesc();
